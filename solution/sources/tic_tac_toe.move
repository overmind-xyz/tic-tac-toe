...
module overmind::tic_tac_toe {
    ...
    fun init_module(admin: &signer) {
        ...
        move_to(
            ...
            State {
                ...
                game_count: 0, 
                games: vector<Game>[], 
                game_created_events: account::new_event_handle(&resource_account_signer),
                game_won_events: account::new_event_handle(&resource_account_signer),
                game_tied_events: account::new_event_handle(&resource_account_signer),
                game_round_events: account::new_event_handle(&resource_account_signer),
                game_expired_events: account::new_event_handle(&resource_account_signer)
            }
        );
    }
    ...
    */
    public entry fun create_game(
        ...
        vector::push_back<Game>(&mut state.games, new_game);
    ...
    */ 
    public entry fun mark_space(
        ...
        check_if_user_is_player(&signer::address_of(player), game);
        ...
        check_if_it_is_players_turn(&signer::address_of(player), game);
        ...
        let space_to_mark = vector::borrow_mut(
            vector::borrow_mut(&mut game.spaces, (row_index as u64)), 
            (column_index as u64)
        );
        ...
        if (player_has_won(&game.spaces, row_index, column_index, game.current_turn)) {
            option::fill(&mut game.result, game.current_turn);
            ...
            event::emit_event(
                ...
                GameWonEvent {
                    game_id: game.game_id,
                    winner: signer::address_of(player), 
                    event_creation_timestamp_seconds: timestamp::now_seconds()
                }
            );
        ...
        if (game_spaces_are_full(&game.spaces)) {
            option::fill(&mut game.result, GAME_RESULTS_TIE);
            coin::transfer<AptosCoin>(
                &account::create_signer_with_capability(&state.signer_cap),
                game.creator, 
                PRIZE_AMOUNT_APT
        ...
        game.expiration_timestamp_seconds = timestamp::now_seconds() + DURATION_ROUND;
    }
    ...
    */  
    public entry fun release_funds_after_expiration(game_id: u64) acquires State {
        ...
        let player_to_reward = if (game.current_turn == PLAYER_ONE) {
            game.player_two
        } else {
            game.player_one
        };
        ...
        option::fill(&mut game.result, GAME_RESULTS_EXPIRED);
        ...
        event::emit_event(
            ...
            GameExpiredEvent {
                game_id: game.game_id, 
                prize_recipient: player_to_reward, 
                event_creation_timestamp_seconds: timestamp::now_seconds()
            }
        );
    ...
    */  
    inline fun player_has_won(
        ...
        row_is_filled_by_player(spaces, row_index, player) || 
        col_is_filled_by_player(spaces, column_index, player) ||
        diagonal_is_filled_by_player(spaces, player)
    }
    ...
    */  
    inline fun row_is_filled_by_player(
        ...
        let i = 0;
        while (i < 3) {
            let space = vector::borrow(row, i);
            if (!(option::is_some(space) && option::contains(space, &player))) {
                is_row_filled = false;
                break
            };
            i = i + 1;
    ...
    */  
    inline fun col_is_filled_by_player(
        ...
        let is_col_filled = true;

        // TODO: Return true if the spaces of the specified column are all filled by the specified 
        //          player and return false otherwise
        let i = 0;
        while (i < 3) {
            let row = vector::borrow(spaces, i);
            let space = vector::borrow(row, (column_index as u64));
            if (!(option::is_some(space) && option::contains(space, &player))) {
                is_col_filled = false;
                break
            };
    ...
    */  
    inline fun diagonal_is_filled_by_player(
        ...
        left_to_right_diagonal_is_filled_by_player(spaces, player) || 
            right_to_left_diagonal_is_filled_by_player(spaces, player)
    }
    ...
    */  
    ...
    inline fun check_if_user_has_enough_apt_coin(user: address, amount_of_apt: u64) {
        ...
        assert!(
            coin::balance<AptosCoin>(user) >= amount_of_apt, 
            EInsufficientAptBalance
        );
    }
    ...
    inline fun check_if_user_is_player(user: &address, game: &Game) {
        ...
        //          EUserNotPlayer
        assert!(
            user == &game.player_one || user == &game.player_two, 
            EUserNotPlayer
        );
    }
    ...
    inline fun check_if_game_has_no_result(game: &Game) {
        ...
        assert!(
            option::is_none(&game.result) == true, 
            EGameHasResult
        );
    }
    ...
    inline fun check_if_it_is_players_turn(player: &address, game: &Game) {
        ...
        assert!(
            (player == &game.player_one && game.current_turn == PLAYER_ONE) || 
                (player == &game.player_two && game.current_turn == PLAYER_TWO),
            ENotPlayersTurn
        );
    }
    ...
    inline fun check_if_turn_is_expired(game: &Game) {
        ...
        //          code: ETurnIsExpired
        assert!(
            game.expiration_timestamp_seconds > timestamp::now_seconds(),
            ETurnIsExpired
        );
    }
    ...
    inline fun check_if_game_is_expired(expiration_timestamp_seconds: u64) {
        ...
        //          EGameIsNotExpired
        assert!(
            expiration_timestamp_seconds < timestamp::now_seconds(), 
            EGameIsNotExpired
        );
    }
